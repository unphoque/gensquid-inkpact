...
const showAllBM=async function(interaction){
    ...
    let sql=`SELECT ca.NAME as CARDNAME, ca.COLLECTION as COLLEC, b.PRICE as PRICE, p.NAME as PLAYERNAME FROM CARDS ca, BLACKMARKET b, PLAYERS p WHERE ca.ID=b.CARDID AND b.OWNERID=p.ID`
    await db.select(sql,(res)=>{
        ...
        for (let i = 0; i < res.length; i++) {
            let c=res[i]
            desc+=`${c.PLAYERNAME} - ${c.COLLEC} ${c.CARDNAME} - ${c.PRICE}\n`
        }
        embed.setDescription(desc)
        interaction.editReply({embeds:[embed]})
    });
